...
variable "domain_name" {
  description = "Base domain name for the environment (e.g., stage.rvdevops.com)"
  type        = string
}
...
variable "AUTH0_CALLBACK_URL" {
  description = "Auth0 callback URL after authentication"
  type        = string
}
...
variable "FARMING_DB_HOST" {
  description = "Host of the farming practices database (located in Dev VPC)"
  type        = string
  default     = ""
}
...
variable "certificate_domain" {
  description = "Domain name for the ACM certificate (e.g. *.rvdevops.com)"
  type        = string
  default     = ""
}
