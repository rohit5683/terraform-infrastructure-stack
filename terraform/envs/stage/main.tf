...
# Stage environment network. Private subnets for ECS/DB, Public for Load Balancers.
module "vpc" {
  source               = "../../modules/vpc"
  name                 = "${var.env}-vpc"
  ...
  enable_vpc_endpoints = true           # UPDATED: Private endpoints enabled to match Prod network path
...
# Common execution roles for ECS tasks.
module "iam" {
  source           = "../../modules/iam"
  env              = var.env
  region           = var.aws_region
...
# Security Groups
# Firewall rules for all stage components.
module "sg" {
...
# DNS Records
# Routes 'app.stage...' and 'api.stage...' to the Load Balancer.
module "route53" {
...
# Public facing ALB to handle incoming HTTP/HTTPS traffic.
module "alb" {
  source = "../../modules/alb"
  ...
  # UPDATED: Internal ALB for API Gateway Integration (Prod Parity)
  # This allows the API Gateway to route traffic privately to the backend.
  # UPDATED: Internal ALB for API Gateway Integration (Prod Parity)
  # This allows the API Gateway to route traffic privately to the backend.
...
# Global CDN (Added for Stage/Prod Parity)
# Acts as the ONLY entry point for traffic. The ALB security group is locked to this distribution.
module "cloudfront" {
  ...
  certificate_arn = module.acm.cloudfront_certificate_arn # UPDATED: Use us-east-1 cert (Prod Parity)
...
# ==============================================================================
# API GATEWAY (Added for Stage/Prod Parity)
# ==============================================================================
...
# Configuration for Frontend (Nginx/React) and Backend (NestJS) containers.
module "ecs_task" {
...
# AWS Secrets Manager
# Securely stores application configuration.
module "secrets" {
  ...
  secret_values = {
    ...
    FARMING_DB_HOST             = var.FARMING_DB_HOST
...
# ECS Services
# Runs the containers defined in Task Definitions.
module "ecs_service" {
...
# Private RDS instance (Private Subnets). No public access for better security.
module "rds" {
  source = "../../modules/rds"

  env                 = var.env
  create_rds          = true
  subnet_ids          = module.vpc.private_subnets # Private subnets for Stage DB
  rds_sg_id           = module.sg.rds_sg_id
  publicly_accessible = false # SECURE: No public IP
